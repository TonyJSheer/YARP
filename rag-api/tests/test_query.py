...
def test_query_citation_fields(client: TestClient) -> None:
...
def test_query_excerpt_is_truncated(client: TestClient) -> None:
    long_text = "A" * 500
...
def test_query_empty_chunks_still_returns_answer(client: TestClient) -> None:
    with (
        patch("app.services.embedding.embed_query", return_value=FAKE_VECTOR),
...
def test_query_invalid_json_returns_error(client: TestClient) -> None:
...
def test_query_stream_returns_sse_content_type(client: TestClient) -> None:
    with (
        patch("app.services.embedding.embed_query", return_value=FAKE_VECTOR),
        patch("app.services.retrieval.retrieve", return_value=[]),
        patch(
...
def test_query_stream_done_event_is_last(client: TestClient) -> None:
    tokens = ["Hello", " world", "[DONE]"]
    with (
        patch("app.services.embedding.embed_query", return_value=FAKE_VECTOR),
        patch("app.services.retrieval.retrieve", return_value=[]),
